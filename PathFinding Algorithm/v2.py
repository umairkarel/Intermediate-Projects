...
class Spot:
    ...
    def show(self, color):
        ...
        pygame.draw.rect(screen, color, pygame.Rect(x1,y1,w,w), 0)
        pygame.draw.rect(screen, black, pygame.Rect(x1,y1,w,w), 1)
    ...
    for i in range(N):
        for j in range(N):
            if board[i][j].obstacle:
                board[i][j].show(black)
            else:
                board[i][j].show(off_white)

    for i in range(len(closedSet)):
        closedSet[i].show(yellow)

    for i in range(len(openSet)):
        openSet[i].show(white)
